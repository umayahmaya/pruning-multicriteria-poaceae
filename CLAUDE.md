...
9. **SELESAI (2026-08-26), didokumentasikan di sini 2026-09-10, diperbarui 2026-09-11 setelah validasi multi-seed.** Kriteria kedua I(c) diganti dari S_BN ke S_GM-Ekspansi dan skema bobot diganti dari konstan ke per-rasio -- lihat Bagian 2 (formula aktif) dan Bagian 2a (formula lama, diarsipkan). Hasil seed 42: `outputs/multicriteria_per_rasio.json`. Hasil validasi 3 seed (42/123/2024, `45_multiseed_per_rasio.py`): `outputs/multiseed_per_rasio_results.json`.
   ...
   **Item lama di bagian "masih terbuka", status diperbarui:**
   ...
   - `channel_selection_comparison.json` (Bagian 8 butir 3) dan uji sensitivitas bobot terkait masih hanya membandingkan bobot konstan lama vs lebih lama -- **MASIH TERBUKA**, belum dihitung ulang untuk bobot per-rasio yang baru.

10. **BELUM DIKERJAKAN (dicatat 2026-09-12). Prioritas: SETELAH proposal disetujui, BUKAN untuk kebutuhan sidang proposal.** Validasi multi-seed untuk kriteria tunggal TERBAIK di tiap rasio (L1, GM-Ekspansi, atau Entropi -- mana pun yang tertinggi pada seed 42 di rasio itu), seed 123 dan 2024 (14 run: 7 rasio x 2 seed baru; seed 42 dipakai ulang dari checkpoint ablation kriteria tunggal yang sudah ada di `outputs/tabel_hasil_lengkap.json` / `outputs/ablation_gm_expansion.json`). Mask pemangkasan TIDAK perlu dihitung ulang karena skor L1/GM-Ekspansi/Entropi bersifat deterministik terhadap baseline yang sama (lihat Bagian 2).

    Tujuan: menutup celah perbandingan SEPIHAK pada pengujian H1 (formula I(c) aktif per-rasio dibandingkan kriteria tunggal terbaik per rasio). Simpangan baku multi-seed saat ini hanya tersedia untuk sisi multi-kriteria (Bagian 8 butir 9): dibandingkan terhadap kriteria tunggal terbaik (satu tarikan seed 42 saja), multi-kriteria melebihi std sendiri di 6 dari 7 rasio (lebih rendah di 10/50/60%, lebih tinggi di 30/40/70%; hanya di 20% selisihnya berada di dalam std sendiri) -- tapi karena std kriteria tunggal pembanding belum ada, belum bisa dipastikan apakah selisih itu juga melebihi variabilitas ALAMI kriteria tunggal, atau kriteria tunggal ternyata sama liarnya antar-seed. H2 (titik terendah multi-kriteria, 88,76% di rasio 70%, tidak lebih rendah dari titik terendah kriteria tunggal manapun) TIDAK terdampak celah ini -- selisihnya (+4,19 sampai +5,33 pp terhadap L1/GM/Entropi, sama-sama di rasio 70%) jauh melampaui simpangan baku terbesar yang pernah terukur di proyek ini (1,43 pp di rasio 50%), sehingga H2 tetap didukung kuat terlepas dari status butir ini. Perhitungan H1/H2 di atas dilakukan ad-hoc dalam percakapan 12 September 2026 dari `outputs/tabel_hasil_lengkap.json` + `outputs/ablation_gm_expansion.json` + `outputs/multiseed_per_rasio_results.json`, belum disimpan sebagai skrip/JSON permanen.
